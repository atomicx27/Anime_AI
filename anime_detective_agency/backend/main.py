...
@app.post("/api/solve")
def solve_case(request: CaseRequest):
    if not AGENT:
        raise HTTPException(status_code=500, detail="Detective Agent not initialized")

    result = AGENT.analyze_case(request.case_description)
    return result
