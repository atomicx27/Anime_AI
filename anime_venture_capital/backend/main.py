...
from agent import VentureCapitalAgent
...
async def lifespan(app: FastAPI):
    ...
    if CHARACTERS:
        ...
        AGENT = VentureCapitalAgent(CHARACTERS)
    else:
        print("Warning: Could not load characters from README.md")
    yield

app = FastAPI(title="Anime Venture Capital AI API", lifespan=lifespan)
...
@app.post("/api/pitch")
def evaluate_pitch(request: PitchRequest):
    if not AGENT:
