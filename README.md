...
The **Anime Detective Agency AI** is a web-based, multi-agent application where users can submit a mystery or case file. The AI dynamically assembles a task force of anime characters, assigning them specific investigative roles based on their profiles, to deduce the underlying truth of the case.

#### Folder Location
`anime_detective_agency/`

#### Features
- **Dynamic Task Force Assembly**: The agent selects a subset of characters and assigns roles like "Lead Investigator", "Forensics Expert", or "Profiler".
- **Character-Driven Deductions**: Each character provides a unique analysis of the case guided by their Core Emotion and Unique Quality.
- **Synthesized Conclusion**: A final overarching conclusion is generated by synthesizing the task force's individual deductions.
- **Modern Detective UI**: A sleek, dark-themed frontend built with Tailwind CSS featuring a glassmorphism design, animated terminal logs, and structured deduction cards.
- **Dynamic Character Loading**: Backend API automatically parses the root `README.md` to load character details.
